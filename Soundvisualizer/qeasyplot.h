...
class QEasyPlot : public QOpenGLWidget
{
    Q_OBJECT
public:
...
public slots:
    ...
    void updateSignal(qreal _min, qreal _max, const qreal *_pt, quint32 length);
    void updateSignal(const qreal *_pt, quint32 length);
    ...
    void set_X_step(qreal value);

private:  
    bool open_colorSelectDialog_for(VisualEntity value);
    void draw_coordinateSystem(QPainter &painter);
    ...
    QString m_objectTempString;
    QString m_ambientTempString;
        //visual appearance
    ...
    qreal stepX; // step of trace for horizontal axis
